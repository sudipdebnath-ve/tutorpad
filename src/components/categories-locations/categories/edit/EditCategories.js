...
import {getPayrollOverideTypes,updateCategories} from "../../../../services/categoriesService.js";
...
const EditCategories = () => {
  ...
  const [timeList,setTimeList] = useState([
        {label:ListComponent({title:"No Reminder",text:""}),text:"No Reminder",value:"0"},
        {label:ListComponent({title:"1 Hours Before",text:""}),text:"1 Hours Before",value:"1"},
        {label:ListComponent({title:"2 Hours Before",text:""}),text:"2 Hours Before",value:"2"},
        {label:ListComponent({title:"3 Hours Before",text:""}),text:"3 Hours Before",value:"3"},
        {label:ListComponent({title:"1 Day Before",text:""}),text:"1 Day Before",value:"24"},
        {label:ListComponent({title:"2 Day Before",text:""}),text:"2 Day Before",value:"48"},
        {label:ListComponent({title:"Custom",text:""}),text:"Custom",value:"-1"},
    ]);
  ...
  const [eventcatEmailReminderDefaultValue,setEventcatEmailReminderDefaultValue] = useState({});
  ...
  const [eventcatSmsReminderDefaultValue,setEventcatSmsReminderDefaultValue] = useState({});
  ...
  const getCategoriesDetailsHandler = async ()=>{
    ...
    if(res.data)
    {
      ...
      setEventcatSmsReminderDefaultValue(timeList.find((f)=>f.value===res.data.eventcat_sms_reminder));
      setEventcatEmailReminderDefaultValue(timeList.find((f)=>f.value===res.data.eventcat_email_reminder));
      setIsEventcatEmailReminder(timeList.find((f)=>f.value===res.data.eventcat_email_reminder)!==undefined?false:true);
      setIsEventcatSmsReminder(timeList.find((f)=>f.value===res.data.eventcat_sms_reminder)!==undefined?false:true);
    }else{
  ...
  const getPayrollOverideTypesHandller = async ()=>{
   ...
   const list = await res.data.map((e)=>{
      return {label:ListComponent({title:e.po_title,text:e.po_description}),text:e.po_title,value:e.id}
    });
                  ...
                  <div className="formbold-form-wrapper">
                      ...
                      <div className="row">
                        <div className="col-md-12">
                          <InputBox label={"Name"} setValue={setEventcatName} value={eventcatName} />
                        </div>
                      </div>
                      <div className="row">
                        <div className="col-md-6">
                          <ColorSelectBox label={"Colors"} isColorBoxOpen={isColorBoxOpen} setColor={setEventcatColor} color={eventcatColor} setIsColorBoxOpen={setIsColorBoxOpen} />
                        </div>
                        <div className="col-md-6">
                          
                          {
                            eventcatPayrollListData?.find((f)=>f.id===eventcatPayroll)?.po_is_numeric===1?<LabelInputBox inputValue={eventcatPayrollValue} setInputValue={setEventcatPayrollValue} label={"Payroll Override"} onClear={()=>setEventcatPayroll("")} unit={eventcatPayrollListData.find((f)=>f.id===eventcatPayroll)?.po_unit_formatted} />:<SelectBox label={"Payroll Override"} options={eventcatPayrollList} onChangeSelect={setEventcatPayroll}  />
                          }
                      ...
                      <div className="row">
                        <div className="col-md-6">
                          {
                            isEventcatEmailReminder?<LabelInputBox inputValue={eventcatEmailReminder} setInputValue={setEventcatEmailReminder} label={"Email Reminders"} onClear={()=>setIsEventcatEmailReminder(false)} unit={"Hours Before"} />:<SelectBox label={"Email Reminders"} selectValue={eventcatEmailReminderDefaultValue}  options={timeList} onChangeSelect={eventcatEmailReminderOnchange}  />
                          }
                        </div>
                        <div className="col-md-6">
                          {
                            isEventcatSmsReminder?<LabelInputBox inputValue={eventcatSmsReminder} setInputValue={setEventcatSmsReminder} label={"SMS Reminders"} onClear={()=>setIsEventcatSmsReminder(false)} unit={"Hours Before"} />:<SelectBox label={"SMS Reminders"} selectValue={eventcatSmsReminderDefaultValue} options={timeList} onChangeSelect={eventcatSmsReminderOnChange}  />
                          }
                      ...
                      <div className="row">
                        <div className="col-md-12">
                          <InputCheckBox isChecked={eventcatMakeupCredits} onChange={makeUpCreditOnchange} label={"Make-Up Credit"} title={"This category blocks other recurring events (e.g. vacation)"} description={""}/>
                          <InputCheckBox isChecked={eventcatBlockedDates} onChange={blockedDatesOnchange} label={"Blocked Dates"} title={"Allow make-up credits to be used for this event type"} description={"This category prevents new events from being added on the same day/time (e.g. vacation) Existing events on the calendar will not be deleted when events in this category are added"}/>
                        </div>
                      </div>
                      <div className="row">
                          <div className="col-md-12">
